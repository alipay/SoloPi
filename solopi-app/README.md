...
> SoloPi新增鸿蒙版本，欢迎大家试用，切到 solopi-harmony分支
...
SoloPi拥有录制操作的能力，用户只需要通过SoloPi执行用例步骤，SoloPi就能够将用户的操作记录下来，并且支持在各个设备上进行回放，这一切都能够在手机上独立完成。详见[录制回放](#录制回放)一节。
...
除了常规性能指标，SoloPi还提供了启动耗时计算工具，测试同学只需要点击两次按钮，就可以得到最贴近用户体验的启动耗时数据。同时，启动耗时计算工具还可以通过广播调用，可以非常方便的与UI自动化测试打通。详见[性能工具](#性能工具)一节。
...
SoloPi支持通过操作一台主机设备来控制多台从机设备，不需要在各个设备上分别进行重复冗杂的兼容性测试，能够极大提升兼容性测试的效率。详见[一机多控](#一机多控)一节。
...
**下载打包好的SoloPi APK（SoloPi.apk文件），或者clone源码在本地编译，具体操作可以参考本页[快速开始](#快速开始)。**



### 文档

- 如果你是第一次使用 SoloPi，推荐先阅读本页[快速开始](#快速开始)
- 项目文档首页：[SoloPi README](../README.md)
...
![SoloPi 体验与交流群（三群）](assets/dingtalk-group.png)

- 微信群（SoloPi 体验与交流群 2.0）：

![SoloPi 体验与交流群 2.0](assets/wechat-group.png)

- 除了交流群外，我们在 TesterHome 也有相关板块，可以在社区里留言回复：<https://testerhome.com/topics/node152>
