...
public class EventService implements ExportService, AppGuardian {
    private static final Object LEGACY_TOUCH_OWNER = new Object();

    private TouchEventTracker touchTracker;
    private final Set<Object> touchTrackerOwners = Collections.newSetFromMap(
    ...
    public void startTrackTouch() {
        startTrackTouch(LEGACY_TOUCH_OWNER);
    }

    public synchronized void startTrackTouch(Object owner) {
        if (owner == null) {
            throw new IllegalArgumentException("Touch tracker owner is required");
        }
        touchTrackerOwners.add(owner);
    ...
    public void stopTrackTouch() {
        stopTrackTouch(LEGACY_TOUCH_OWNER);
    }

    public synchronized void stopTrackTouch(Object owner) {
        if (owner == null) {
            return;
        }
        if (!touchTrackerOwners.remove(owner)) {
            return;
        }
        if (!touchTrackerOwners.isEmpty()) {
            return;
        }
        if (touchTracker != null) {
    ...
    @Override
    public synchronized void onEventTrigger(ReceiveSystemEvent event) {
